...
class C_Prescription extends Controller
{
    ...
    //     Administration->Globals->Rx
        ...
        $pdf->ezImage($GLOBALS['oer_config']['prescriptions']['logo'], null, '20', '', 'center', '');
        $pdf->ezText('', 10);
    }
    ...
    function multiprintcss_header($p)
    {
        ...
        // University heading at the very top, centered 
        // echo("<div style='text-align:center; font-size:14pt; font-weight:bold; margin-bottom:5pt;'>"); 
        // echo("UNIVERSITY OF THE PHILIPPINES VISAYAS"); 
        // echo("</div>\n");
        // // Current header block centered 
        // echo ("<div style='text-align:center; margin-bottom:10pt;'>\n"); 
        // $res = sqlQuery("SELECT concat('<b>',f.name,'</b>\n',f.street,'\n',f.city,', ',f.state,' ',f.postal_code,'\nTel:',f.phone) addr FROM users JOIN facility AS f ON f.name = users.facility WHERE users.id ='" . add_escape_custom($p->provider->id) . "'"); 
        // if (!empty($res)) 
        //     { 
        //         $patterns = array ('/\n/','/Tel:/'); 
        //         $replace = array ('<br />', xl('Tel') . ':'); 
        //         $res = preg_replace($patterns, $replace, $res); 
        //         } 
        //         echo ('<span class="large">' . ($res['addr'] ?? '') . '</span>'); 
        //         echo ("</div>\n");
                
echo("<div style='text-align:center; font-size:14pt; font-weight:bold; margin-bottom:1pt;'>");
echo("UNIVERSITY OF THE PHILIPPINES VISAYAS<br />");
echo("</div>\n");

echo("<div style='text-align:center; margin-bottom:5pt;'>\n");

$res = sqlQuery("SELECT f.name, f.street, f.city, f.state, f.postal_code, f.phone
                 FROM users JOIN facility AS f ON f.name = users.facility
                 WHERE users.id ='" . add_escape_custom($p->provider->id) . "'");

if (!empty($res)) {
    // Facility name on its own line
    echo("<div><b>" . htmlspecialchars($res['name']) . "</b><br /></div>\n");

    // City/State/Postal on its own line
    echo("<div>" . htmlspecialchars($res['city'] . ', ' . $res['state'] . ' ' . $res['postal_code']) . "<br /></div>\n");

    // Tel on its own line, then break
    echo("<div>Tel: " . htmlspecialchars($res['phone']) . "<br /></div>\n");
}
...
echo("</div>\n");
                  ...
                  // Patient info table with Name, Date, Address, Age/Sex
    ...
    $age = getPatientAge($res['DOB']);
    $sex = $res['sex'];

    echo ("<table cellspacing='0' cellpadding='4' width='100%' style='border-collapse:collapse;'>\n");
    echo ("<tr>");
    echo ("<td style='border-bottom:1px solid black;'><b>Name:</b> " . htmlspecialchars($p->patient->get_name_display()) . "</td>");
    echo ("<td style='border-bottom:1px solid black;'><b>Date:</b> " . date('Y-m-d') . "</td>");
    echo ("</tr>\n");

    echo ("<tr>");
        echo ("<td style='border-bottom:1px solid black;'><b>Address:</b> " . htmlspecialchars($address) . "</td>");
    echo ("<td style='border-bottom:1px solid black;'><b>Age/Sex:</b> " . htmlspecialchars($age . ' / ' . $sex) . "</td>");
    echo ("</tr>\n");
    echo ("</table><br>\n");


    echo ("<img WIDTH='30pt' src='./interface/pic/" . $GLOBALS['oer_config']['prescriptions']['logo_pic'] . "' />");
...
function multiprintcss_preheader()
{
    ...
    echo ("div.scriptdiv {\n");
    echo (" padding-top: 10pt;\n");
    echo (" padding-bottom: 15pt;\n");
    echo (" padding-left: 15pt;\n");
    echo (" border-bottom:1pt solid black;\n");
    echo ("}\n");

    echo ("div.signdiv {\n");
    echo (" margin-top: 25pt;\n");
    echo (" font-size: 10pt;\n");
    echo ("}\n");
    ...
    function multiprint_footer(&$pdf, $p)
    {
        ...
        $pdf->ezText('<b>' . $p->provider . '</b>', 12, array('justification'=>'center'));
...
// exit;
    ...
    // Provider Name
    echo ("<b>" . htmlspecialchars($p->provider->fname) ." ". htmlspecialchars($p->provider->lname) . "</b><br />");
echo ("<b>" . xl('State Lic. #') . ":</b> " . htmlspecialchars($p->provider->state_license_number) . "<br />"); 
echo ("<b>" . xl('PTR #') . ":</b> " . str_pad($p->patient->get_pubpid(), 10, "0", STR_PAD_LEFT) . "<br />");
...
function get_prescription_body_text($p)
{
    ...
    // Only show Rx if drug name is not blank
    if ($p->get_drug() != '') {
        $body .= '<b>' 
              . text($p->get_drug()) . ' ' 
              . text($p->get_size()) . ' ' 
              . text($p->get_unit_display());

        if ($p->get_form()) {
            $body .= ' [' . text($p->form_array[$p->get_form()]) . "]";
        }

        $body .= "</b>     <i>" . text($p->substitute_array[$p->get_substitute()]) . "</i>\n";
    }

    // Only show Disp # if quantity > 0
    if ($p->get_quantity() > 0) {
        $body .= '<b>' . xlt('Disp #') . ':</b> <u>' . text($p->get_quantity()) . "</u>\n";
    }
