...
function multiprintcss_header($p)
{
    ...
    // Rx logo + prescription details aligned
    $d = $this->get_prescription_body_text($p);
    $patterns = array ('/\n/','/     /');
    $replace = array ('<br />','&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
    $d = preg_replace($patterns, $replace, $d);

    echo("<table style='width:100%; margin-top:10pt;'>");
    echo("<tr>");
    echo("<td style='width:15%; text-align:center; vertical-align:top;'>");
    echo ("<img width='68pt' src='./interface/pic/" . $GLOBALS['oer_config']['prescriptions']['logo_pic'] . "' />");
    echo("</td>");
    echo("<td style='width:85%; vertical-align:top;'>");
    echo("<div class='scriptdiv'>" . $d . "</div>");
    echo("</td>");
    echo("</tr>");
    echo("</table>");

    // Footer (provider at bottom)
...
function multiprint_header(&$pdf, $p)
{
    ...
    // Patient details
    $res = sqlQuery("SELECT street, city, state, postal_code, DOB, sex 
                     FROM DOB WHERE pid = ?", array($p->patient->id));
    $address = trim($res['street'] . ', ' . $res['city'] . ', ' . $res['state'] . ' ' . $res['postal_code']);
    $age = getPatientAge($res['DOB']);
    $sex = $res['sex'];
   ...
   function multiprintcss_preheader()
{
    ...
    echo ("body { font-family: Arial, sans-serif; font-size: 11pt; margin: 0; }\n");

    /* Container sized to 1/4 A4 */
    echo ("div.paddingdiv { width: 148mm; height: 105mm; margin: auto; padding: 8mm; box-sizing: border-box; }\n");

    /* Header */
    echo ("div.header { text-align: center; font-size: 14pt; font-weight: bold; margin-bottom: 10pt; }\n");

    /* Patient info */
    echo ("table.patientinfo { width: 100%; margin-bottom: 15pt; font-size: 11pt; border-collapse: collapse; }\n");
    echo ("table.patientinfo td { padding: 4pt; vertical-align: top; }\n");

    /* Rx block */
    //echo ("div.rx { font-size: 18pt; font-weight: bold; margin-bottom: 8pt; text-align: left; }\n");
    //echo ("div.prescription { min-height: 80pt; font-size: 12pt; text-align: left; }\n");

    /* Footer */
    echo ("div.footer { position: absolute; bottom: 20mm; left: 8mm; font-size: 11pt; }\n");
    echo ("span.small { font-size: 8pt; }\n");

    echo ("</style>\n<title>" . xl('Prescription') . "</title>\n</head>\n<body>\n");
}
