...
//     // UPV Health Service Unit header
//     $pdf->ezText('<b>UNIVERSITY OF THE PHILIPPINES VISAYAS</b>', 14, array('justification'=>'center'));
//     $pdf->ezText('Health Service Unit', 12, array('justification'=>'center'));
//     $pdf->ezText('Miagao, Iloilo', 12, array('justification'=>'center'));
//     $pdf->ezText('', 10);

//     $res = sqlQuery("SELECT  concat(street,'\n',city,', ',state,' ',postal_code,'\n',if(phone_home!='',phone_home,if(phone_cell!='',phone_cell,if(phone_biz!='',phone_biz,'')))) addr from patient_data where pid =" . add_escape_custom($p->patient->id));
//     $age = getPatientAge($res['DOB']); // helper function for age 
//     $sex = $res['sex'];
//     // Patient details
//     $pdf->ezText('Name: ' . $p->patient->get_name_display(), 10);
//     $pdf->ezText('Addresses: ' . $res['addr'], 10);
//     $pdf->ezText('Date: ' . date('Y-m-d'), 10);
//     $pdf->ezText('Age/Sex: ' . $p->patient->age . ' / ' . $p->patient->sex, 10);
//     $pdf->ezText('', 10);

//     // Rx symbol
//     $pdf->ezText('<b>Rx</b>', 16);
//     $pdf->ezText('', 10);

//     // Prescription text
//     $pdf->ezText($p->drug, 12);
//     $pdf->ezText('', 20);

//     // Provider footer
//     $pdf->ezText($p->provider->get_name_display(), 10);
//     $pdf->ezText('Lic. No.: ' . $p->provider->state_license_number, 10);
//     $pdf->ezText('<b>' . xl('PTR No.') . '</b>', 6);
// }
...
function multiprint_header(&$pdf, $p)
{
    ...
    // Patient details
    $res = sqlQuery("SELECT street, city, state, postal_code, date_of_birth, sex 
                     FROM patient_data WHERE pid = ?", array($p->patient->id));
    $address = trim($res['street'] . ', ' . $res['city'] . ', ' . $res['state'] . ' ' . $res['postal_code']);
    $age = getPatientAge($res['date_of_birth']);
    $sex = $res['sex'];

    $pdf->ezText('Name: ' . $p->patient->get_name_display(), 10);
    $pdf->ezText('Address: ' . $address, 10);
    $pdf->ezText('Date: ' . date('Y-m-d'), 10);
    $pdf->ezText('Age/Sex: ' . $age . ' / ' . $sex, 10);
    $pdf->ezText('', 10);
    ...
    $pdf->ezText('Drug: ' . $p->drug, 12);
    $pdf->ezText('Disp #: ' . $p->disp_quantity, 12);
    $pdf->ezText('Sig: ' . $p->sig, 12);
    $pdf->ezText('', 10);

    // Refills and signature line
    $pdf->ezText('Refills: ' . $p->refills, 12);
    $pdf->ezText('Signature: ___________________________', 12);
    $pdf->ezText('Date: ' . date('Y-m-d'), 12);
    $pdf->ezText('', 20);

    // Push provider footer to bottom of page
    $pdf->ezSetY(50); // move cursor near bottom
    $pdf->ezText($p->provider->get_name_display(), 10);
    $pdf->ezText('Lic. No.: ' . $p->provider->state_license_number, 10);
    $pdf->ezText('<b>' . xl('PTR No.') . '</b>', 10);
}
    ...
    //     echo ("<tr>\n");
    //     echo ("<td>\n");
    //     $res = sqlQuery("SELECT concat('<b>',f.name,'</b>\n',f.street,'\n',f.city,', ',f.state,' ',f.postal_code,'\nTel:',f.phone) addr FROM users JOIN facility AS f ON f.name = users.facility where users.id ='" . add_escape_custom($p->provider->id) . "'");
    //     if (!empty($res)) {//,if(f.fax != '',concat('\nFax: ',f.fax),'')
    //         $patterns = array ('/\n/','/Tel:/');//,'/Fax:/'
...
//     echo("<b>UNIVERSITY OF THE PHILIPPINES VISAYAS</b><br>");
//     echo("Health Service Unit<br>");
//     echo("Miagao, Iloilo<br>");
//     echo("</div><br>");

//     // Patient details
// $res = sqlQuery("SELECT street, city, state, postal_code, DOB, sex 
//                  FROM patient_data WHERE pid = ?", array($p->patient->id));

// $address = trim($res['street'] . ', ' . $res['city'] . ', ' . $res['state'] . ' ' . $res['postal_code']);
// $dob = $res['DOB'];
// $age = getPatientAge($dob); // helper function in OpenEMR
// $sex = $res['sex'];

// echo("<table class='patientinfo'>");
// echo("<tr><td>Name: " . htmlspecialchars($p->patient->get_name_display()) . "</td>");
// echo("<td>Date: " . date('Y-m-d') . "</td></tr>");
// echo("<tr><td>Address: " . htmlspecialchars($address) . "</td>");
// echo("<td>Age/Sex: " . htmlspecialchars($age . ' / ' . $sex) . "</td></tr>");
// echo("</table><br>");
...
//     // echo("<tr><td>Name: " . htmlspecialchars($p->patient->get_name_display()) . "</td>");
//     // echo("<td>Date: " . date('Y-m-d') . "</td></tr>");
//     // echo("<tr><td>Address: " . htmlspecialchars($p->patient->street . ', ' . $p->patient->city) . "</td>");
//     // echo("<td>Age/Sex: " . htmlspecialchars($p->patient->age . ' / ' . $p->patient->sex) . "</td></tr>");
//     // echo("</table><br>");
...
//     echo(htmlspecialchars($p->drug)); // adjust field if needed
//     echo("</div><br><br>");

//     // Provider footer
//     echo("<div class='footer'>");
//     echo($p->provider->get_name_display() . "<br>");
//     echo("Lic. No.: " . htmlspecialchars($p->provider->state_license_number). "<br>");
//     echo ('<b><span class="small">' . xl('PTR No.') . '</span></b>' . '<br />');
//     echo("</div>");
...
function multiprintcss_header($p)
{
    ...
    // Rx block with logo and prescription text aligned
    echo("<table style='width:100%; margin-bottom:15pt;'>");
    echo("<tr>");
    echo("<td style='width:20%; text-align:center; vertical-align:top;'>");
    echo ("<img width='68pt' src='./interface/pic/" . $GLOBALS['oer_config']['prescriptions']['logo_pic'] . "' />");
     echo("</td>");
     echo("<td style='width:80%; vertical-align:top;'>");
    // echo("<div class='rx'>Rx</div>");
    // echo("<div class='prescription'>");
    // echo(nl2br(htmlspecialchars($p->drug))); // preserves line breaks
    // echo("</div>");
     echo("</td>");
    echo("</tr>");
    echo("</table>");

    // Provider footer aligned under Rx
    echo("<div class='footer'>");
    echo($p->provider->get_name_display() . "<br>");
    echo("Lic. No.: " . htmlspecialchars($p->provider->state_license_number) . "<br>");
    echo("<b><span class='small'>" . xl('PTR No.') . "</span></b><br>");
...
//     // Header, patient info, Rx, footer styling
//     echo ("div.header { text-align:center; font-size:14pt; font-weight:bold; margin-bottom:10pt; }\n");
//     echo ("table.patientinfo { width:100%; margin-bottom:15pt; font-size:11pt; }\n");
//     echo ("table.patientinfo td { padding:4pt; }\n");
//     echo ("div.rx { font-size:18pt; font-weight:bold; margin-top:20pt; margin-bottom:20pt; }\n");
//     echo ("div.prescription { min-height:80pt; margin-bottom:20pt; }\n");
//     echo ("div.footer { margin-top:20pt; font-size:11pt; }\n");
...
//     echo (" box-sizing: border-box;\n");
//     echo (" border: 1pt solid black;\n"); /* optional border for cutting guides */
//     echo ("}\n");
...
//     echo (" margin-bottom: 10pt;\n");
//     echo ("}\n");

//     /* Patient info block */
//     echo ("table.patientinfo {\n");
//     echo (" width: 100%;\n");
//     echo (" margin-bottom: 15pt;\n");
//     echo (" font-size: 11pt;\n");
//     echo (" border-collapse: collapse;\n");
//     echo ("}\n");
//     echo ("table.patientinfo td {\n");
//     echo (" padding: 4pt;\n");
//     echo (" vertical-align: top;\n");
//     echo ("}\n");
...
//     echo (" font-size: 18pt;\n");
//     echo (" font-weight: bold;\n");
//     echo (" margin-top: 10pt;\n");
//     echo (" margin-bottom: 10pt;\n");
//     echo (" text-align: left;\n");
//     echo ("}\n");

//     /* Prescription text area */
//     echo ("div.prescription {\n");
//     echo (" min-height: 80pt;\n");
//     echo (" margin: 0;\n");
//     echo (" padding-left: 0;\n");
//     echo (" font-size: 12pt;\n");
//     echo (" text-align: left;\n");
...
//  <div class='patient-info'>". $this->get_patient_header_text($p) ."</div> 
//  <div class='prescription-info'>". $d ."</div> 
//  </div>");
//  }
