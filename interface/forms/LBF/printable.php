...
//physician information absed on logged in physician user
$authUser = $_SESSION['authUserID'];
$provider = sqlQuery("SELECT fname,mname, lname, state_license_number,valedictory FROM users WHERE id = ?", array($authUser));
$provider_name = $provider['fname'] . ' ' . $provider['mname']. ' ' . $provider['lname']. ', '. $provider['valedictory'];
$license = $provider['state_license_number'];

function getLbfValue($formid, $field_id){
	$result= sqlQuery("SELECT field_value FROM lbf_data WHERE form_id = ? AND field_id= ?", array($formid, $field_id));
	return $result ? $result['field_value'] : '';
}
$patient_data=getPatientData($patientid);
$form_fname=$patient_data['fname'];
$form_findings=getLbfValue($formid,'findings');
$form_impression=getLbfValue($formid,'impression');
$form_remarks=getLbfValue($formid,'remarks');

?>
<p style="text-align: center; font-size: 14px;">
 <br><br> UNIVERSITY OF THE PHILIPPINES<BR>
 <strong> HEALTH SERVICES UNIT</strong><br>
 Miagao, Iloilo<br>
Tel. No. : (033) 315-8301
</p>
<p style ="text-align: right; font-size: 14px;">
Date:<em><?php echo date('F j, Y'); ?></em>
</p>
<p style="text-align:center; font-size:14px;">
<strong><?php echo text($formtitle); ?></strong>
</p>
<p style ="text-align:justify; font-size:14px;">
TO WHOM IT MAY CONCERN:<br><br>
<?php
$formattedDate=date('M d, Y', strtotime($dateofservice));
?>
This is to certify that <strong><?php echo text($patientname); ?></strong> sought consultation last <strong><?php echo  $formattedData; 
?></strong>.<br><br>
<strong> FINDIMGS: </strong> <?php echo text($form_findings); ?>.<br>
<strong> IMPRESSION: </strong><?php echo text($form_impression); ?>. <br>
<strong> REMARKS: </strong> <?php echo text($form_remarks);?>.<br><br>
This certification is issued upon request for whatever legal purposes it may serve her/him.<br><br>

<p style="text-align: right; font-size: 14px;">
   <strong><?php echo text($provider_name); ?></strong><br>
  License No.: <em><?php echo text($license); ?></em><br>
...
if($formtitle !=='Medical Certificate'){
